//! Domain policy for Course aggregate
//!
...
/// Domain policy for Course — permits all operations (no business invariants).
///
/// To add domain rules, replace this alias with a struct implementing
/// `backbone_core::DomainPolicy<Course>` in the `// <<< CUSTOM` zone.
pub type CourseDomainPolicy = PermitAllPolicy<Course>;
