//! Repository trait for Competency aggregate
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! This trait defines the repository contract for the Competency aggregate.
//! Implementation is in the infrastructure layer.

use async_trait::async_trait;
use anyhow::Result;
use uuid::Uuid;

use crate::domain::entity::{Competency, CompetencyCategory};

/// Pagination parameters for list queries
#[derive(Debug, Clone, Default)]
pub struct CompetencyPaginationParams {
    pub page: u32,
    pub per_page: u32,
}

impl CompetencyPaginationParams {
    pub fn new(page: u32, per_page: u32) -> Self {
        Self { page, per_page }
    }

    pub fn offset(&self) -> u64 {
        ((self.page.saturating_sub(1)) * self.per_page) as u64
    }

    pub fn limit(&self) -> u64 {
        self.per_page as u64
    }
}

/// Paginated result for list queries
#[derive(Debug, Clone)]
pub struct CompetencyPaginatedResult {
    pub data: Vec<Competency>,
    pub total: u64,
    pub page: u32,
    pub per_page: u32,
}

/// Filter parameters for list queries
#[derive(Debug, Clone, Default)]
pub struct CompetencyFilter {
    pub company_id: Option<Uuid>,
    pub name: Option<String>,
    pub category: Option<CompetencyCategory>,
    pub description: Option<String>,
}

impl CompetencyFilter {
    /// Check if any filter is set
    pub fn has_filters(&self) -> bool {
        self.company_id.is_some() || self.name.is_some() || self.category.is_some() || self.description.is_some()
    }
}

/// Repository trait for Competency aggregate
///
/// This trait defines the persistence contract for Competency entities.
/// Implementations should be in the infrastructure layer.
#[async_trait]
pub trait CompetencyRepository: Send + Sync {

    // =========================================================================
    // Core CRUD Operations
    // =========================================================================

    /// Save a new competency entity
    async fn save(&self, entity: &Competency) -> Result<Competency>;

    /// Find competency by ID
    async fn find_by_id(&self, id: &str) -> Result<Option<Competency>>;

    /// Find all competency entities
    async fn find_all(&self) -> Result<Vec<Competency>>;

    /// Update competency by ID
    async fn update(&self, id: &str, entity: &Competency) -> Result<Option<Competency>>;

    /// Delete competency by ID
    async fn delete(&self, id: &str) -> Result<bool>;

    // =========================================================================
    // Pagination
    // =========================================================================

    /// List competency with pagination
    async fn list(&self, params: CompetencyPaginationParams) -> Result<CompetencyPaginatedResult>;

    /// List competency with pagination and filters
    async fn list_with_filters(&self, params: CompetencyPaginationParams, filters: CompetencyFilter) -> Result<CompetencyPaginatedResult>;

    /// Count all competency entities
    async fn count(&self) -> Result<u64>;

    /// Count competency entities matching filters
    async fn count_with_filters(&self, filters: CompetencyFilter) -> Result<u64>;

    /// Check if competency exists by ID
    async fn exists(&self, id: &str) -> Result<bool>;

    // =========================================================================
    // Soft Delete Operations
    // =========================================================================

    /// Soft delete competency by ID
    async fn soft_delete(&self, id: &str) -> Result<bool>;

    /// Restore soft-deleted competency by ID
    async fn restore(&self, id: &str) -> Result<Option<Competency>>;

    /// List soft-deleted competency entities
    async fn list_deleted(&self, params: CompetencyPaginationParams) -> Result<CompetencyPaginatedResult>;

    /// Empty trash (permanently delete all soft-deleted entities)
    async fn empty_trash(&self) -> Result<u64>;

    // =========================================================================
    // Bulk Operations
    // =========================================================================

    /// Bulk save competency entities
    async fn bulk_save(&self, entities: &[Competency]) -> Result<Vec<Competency>>;

    /// Bulk delete by IDs
    async fn bulk_delete(&self, ids: &[&str]) -> Result<u64>;
}
